  ...
  useEffect(() => {
    ...
    const render = () => {
        ...
        particles.forEach((p, i) => {
            ...
            const safeZ = perspective + p.z;
            if (safeZ <= 1) return; // Prevent division by zero or points behind camera

            const scaleFactor = perspective / safeZ;
            const x2d = cx + p.x * scaleFactor;
            const y2d = cy + p.y * scaleFactor;
            const size = Math.max(0.1, (isThinking ? 4 : 2.5) * scaleFactor);

            if (!Number.isFinite(x2d) || !Number.isFinite(y2d) || !Number.isFinite(size)) return;

            const alpha = Math.max(0, Math.min(1, (p.z + radius) / (2 * radius))); // Fade back particles

            ctx.beginPath();
            ctx.arc(x2d, y2d, size, 0, Math.PI * 2);
            // Gradient fill
            const gradRadius = Math.max(0.1, size * 3);
            if (Number.isFinite(gradRadius) && gradRadius > 0 && Number.isFinite(x2d) && Number.isFinite(y2d)) {
                try {
                    const grad = ctx.createRadialGradient(x2d, y2d, 0, x2d, y2d, gradRadius);
            ...
            if (!isThinking) {
                 particles.slice(i+1, i+4).forEach(p2 => {
                     ...
                     if (dist < 45) {
                        const safeZ2 = perspective + p2.z;
                        if (safeZ2 <= 1) return;
                        const s2 = perspective / safeZ2;
                        const x2d2 = cx + p2.x * s2;
                        const y2d2 = cy + p2.y * s2;
                        
                        if (Number.isFinite(x2d2) && Number.isFinite(y2d2)) {
                            ctx.beginPath();
                            ctx.moveTo(x2d, y2d);
        ...
        if (Number.isFinite(glowSize) && glowSize > 0 && Number.isFinite(cx) && Number.isFinite(cy)) {
            try {

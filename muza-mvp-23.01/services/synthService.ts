...
class MuzaSynth {
    ...
    public playGenerativeTrack(hyperbit: HyperBit) {
        if (!this.audioContext || !this.masterGain) return;
        this.stopGenerativeMusic();

        const optics = hyperbit.optics;
        const color = optics.baseColor;
        ...
        if (r > g + b) { // Red dominant (Fire) -> Major, high pitch
            scale = SCALES['MAJOR_PENTATONIC'];
            rootNote = 72; // High C
            waveform = 'sawtooth';
        } else if (b > r + g) { // Blue dominant (Water) -> Minor, low pitch
            scale = SCALES['AEOLIAN'];
            rootNote = 48; // Low C
            waveform = 'triangle';
        } else { // Others -> Neutral/Mixolydian
            scale = SCALES['MIXOLYDIAN'];
            rootNote = 60; // Middle C
        }

        const bpm = 70 + (hyperbit.energy * 100);
        ...
        this.musicInterval = setInterval(() => {
            ...
            // Use Golden Ratio for melodic leaps (Fibonacci-like jumps)
            ...
            gainNode.gain.linearRampToValueAtTime(0.15 * optics.brightness, this.audioContext.currentTime + noteDuration * 0.1);

...
import { EMOTION_LABELS, TRANSLATIONS, TYPE_COLORS } from '../constants';
...
// Physics configurations for different emotions
const EMOTION_PHYSICS: Record<EmotionType, { flowX: number, flowY: number, turbulence: number, speed: number }> = {
    [EmotionType.NEUTRAL]:     { flowX: 0.2,  flowY: 0,    turbulence: 0.1, speed: 0.5 }, // Gentle drift
    [EmotionType.CURIOUS]:     { flowX: 0.5,  flowY: 0.5,  turbulence: 0.3, speed: 1.0 }, // Diagonal exploration
    [EmotionType.HAPPY]:       { flowX: 0,    flowY: -1.0, turbulence: 0.2, speed: 1.5 }, // Rising bubbles (Up)
    [EmotionType.EXCITED]:     { flowX: 0,    flowY: -2.0, turbulence: 0.8, speed: 2.5 }, // Rapid ascent, chaotic
    [EmotionType.THOUGHTFUL]:  { flowX: 0.1,  flowY: 0,    turbulence: 0.05, speed: 0.2 }, // Very slow, static
    [EmotionType.INSPIRED]:    { flowX: 0.8,  flowY: -0.8, turbulence: 0.5, speed: 2.0 }, // Fast diagonal
    [EmotionType.MELANCHOLIC]: { flowX: 0,    flowY: 0.5,  turbulence: 0.1, speed: 0.3 }, // Slow rain (Down)
};
...
export const MatrixVision: React.FC<MatrixVisionProps> = ({ state, hyperbits, language = 'en' }) => {
    ...
    const h = TRANSLATIONS[language].matrixHints;
    ...
    const emitters = useMemo(() => {
        return hyperbits.slice(-8).map(hb => {
            ...
            const hex = optics?.baseColor || TYPE_COLORS[hb.type] || '#22d3ee';
    ...
    const startCapture = async () => {
        try {
            ...
            console.warn(isRu ? "Захват экрана отменён или запрещён" : "Screen capture cancelled or denied", e);
            // Gracefully reset state so the button works again
            setIsSharing(false);
        }
    };
    ...
    return (
        <div className="relative w-full h-full bg-black overflow-hidden font-mono">
                ...
                <div className="absolute inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in">
                    <div className="bg-slate-900 border border-cyan-500/50 p-6 rounded-2xl max-w-md shadow-[0_0_50px_rgba(34,211,238,0.2)] relative">
                        ...
                        <div className="text-xs text-slate-500 font-mono">
                            <div className="mb-1">{isRu ? 'СОСТОЯНИЕ' : 'STATE'}: {isSharing ? (isRu ? 'ВХОДЯЩИЙ_ПОТОК' : 'INPUT_STREAM') : (isRu ? 'ВЫХОДЯЩИЙ_ПОТОК' : 'OUTPUT_STREAM')}</div>
                            <div>{isRu ? 'РАЗРЕШЕНИЕ' : 'RESOLUTION'}: {resolution}px</div>
                        </div>
                    </div>
                </div>
            )}
            ...
            {/* HUD */}
            <div className="absolute top-6 left-1/2 -translate-x-1/2 flex items-center gap-4 p-4 bg-black/80 backdrop-blur-md rounded-2xl border border-white/10 z-50 transition-all hover:border-white/30 shadow-2xl">
                <div className="flex items-center gap-2 text-white">
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-[8px] text-slate-500 uppercase leading-none mt-1">
                            {isSharing ? h.videoFeed : `${h.outputStream}: ${EMOTION_LABELS[language][state.activeEmotion]}`}
                        </span>
                    </div>
                </div>
                ...
                {!isSharing ? (
                    <Tooltip content={h.activate} position="bottom">
                    ...
                    <Tooltip content={h.stop} position="bottom">
            ...
            {!isSharing && (
                <div className="absolute bottom-6 left-6 p-4 bg-black/40 backdrop-blur-sm rounded-xl border border-white/5 text-[10px] text-slate-500 max-w-sm pointer-events-none z-50">
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 font-mono">
                        ...
                        <span className="text-slate-400">{isRu ? 'ЭНЕРГИЯ' : 'ENERGY'}:</span>
                        <span className={state.energyLevel > 0.8 ? 'text-red-400' : 'text-slate-300'}>{(state.energyLevel * 100).toFixed(0)}%</span>
                    </div>
                </div>
            )}

            {hyperbits.length === 0 && !isSharing && (
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-xs text-slate-500 bg-black/40 border border-white/10 px-4 py-2 rounded-full pointer-events-none z-40">
                    {h.noData}
                </div>
            )}

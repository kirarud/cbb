...
import { Language, MusicTrack, EmotionType, MuzaState } from '../types';
import { synthService } from '../services/synthService';
import { EMOTION_LABELS, EMOTION_TO_SCALE, TRANSLATIONS } from '../constants';
...
export const MusicLab: React.FC<MusicLabProps> = ({ language, onLog, state }) => {
    ...
    const buildDefaultPlaylist = (lang: Language): MusicTrack[] => {
        const ru = lang === 'ru';
        return [
            { id: '1', title: ru ? 'Нейро‑Генезис' : 'Neural Genesis', artist: ru ? 'Ядро Музы' : 'Muza Core', source: 'GENERATED', duration: '∞', emotion: EmotionType.CURIOUS },
            { id: '2', title: ru ? 'Эхо Пустоты' : 'Void Echoes', artist: ru ? 'Муза (Глубокий режим)' : 'Muza (Deep Mode)', source: 'GENERATED', duration: '∞', emotion: EmotionType.MELANCHOLIC }
        ];
    };

    const [playlist, setPlaylist] = useState<MusicTrack[]>(() => buildDefaultPlaylist(language));

    const currentTrack = playlist[currentTrackIndex];
    const theoryInfo = currentTrack.source === 'GENERATED' ? EMOTION_TO_SCALE[currentTrack.emotion] : null;
    const t = TRANSLATIONS[language].musicLab;

    useEffect(() => {
        setPlaylist(prev => prev.map(track => {
            if (track.id === '1' && track.source === 'GENERATED') {
                return { ...track, title: isRu ? 'Нейро‑Генезис' : 'Neural Genesis', artist: isRu ? 'Ядро Музы' : 'Muza Core' };
            }
            if (track.id === '2' && track.source === 'GENERATED') {
                return { ...track, title: isRu ? 'Эхо Пустоты' : 'Void Echoes', artist: isRu ? 'Муза (Глубокий режим)' : 'Muza (Deep Mode)' };
            }
            return track;
        }));
    }, [isRu]);
    ...
    useEffect(() => {
        if (currentTrack.source === 'GENERATED' && isPlaying && currentTrack.emotion !== state.activeEmotion) {
            setPlaylist(prev => prev.map((p, idx) => idx === currentTrackIndex ? { ...p, emotion: state.activeEmotion } : p));
        }
    }, [state.activeEmotion, isPlaying, currentTrackIndex, currentTrack.source]);
    ...
    // --- AUDIO HANDLING ---
    useEffect(() => {
        // Stop any previously running audio for this module
        if (!isPlaying) {
        ...
        if (currentTrack.source === 'GENERATED') {
            ...
            audioRef.current.play().catch(() => {
                onLog(isRu ? "Ошибка воспроизведения внешней ссылки." : "Error playing external link.");
                setIsPlaying(false);
            });
        }
    ...
    const generateNewTrack = () => {
        ...
        const emotionLabel = EMOTION_LABELS[language][randomEmo];
        const newTrack: MusicTrack = {
            id: Date.now().toString(),
            title: isRu ? `Нейро‑Опус #${Math.floor(Math.random() * 1000)}` : `Neural Opus #${Math.floor(Math.random() * 1000)}`,
            artist: isRu ? 'Муза ИИ' : 'Muza AI',
            source: 'GENERATED',
            duration: '∞',
            emotion: randomEmo
        };
        setPlaylist([...playlist, newTrack]);
        onLog(isRu ? `Создан новый трек. Эмоция: ${emotionLabel}` : `Generated new track with emotion: ${emotionLabel}`);
    };
            ...
            <div className="flex-1 relative bg-black">
                ...
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none mix-blend-difference">
                    <h2 className="text-4xl md:text-6xl font-black text-white tracking-tighter uppercase opacity-80 animate-pulse">
                        {isPlaying ? currentTrack.title : t.paused}
                    </h2>
                    ...
                    {isPlaying && theoryInfo && (
                        <div className="mt-4 p-2 border border-white/20 rounded-lg inline-block bg-black/40 backdrop-blur-sm">
                            <div className="text-xs text-cyan-400 font-mono">{t.theoryActive}</div>
                            <div className="text-sm text-white font-bold">{theoryInfo.scale} {t.scaleLabel} • {theoryInfo.bpm} {t.bpmLabel}</div>
                        </div>
                    )}
                </div>
                ...
                <div className="absolute top-0 right-0 h-full w-80 bg-slate-900/80 backdrop-blur-md border-l border-slate-700 transform translate-x-full hover:translate-x-0 transition-transform duration-300 z-10 flex flex-col">
                    <div className="p-4 border-b border-slate-700 bg-slate-900 flex justify-between items-center">
                        <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t.queue}</span>
                        <span className="text-xs text-slate-600">{playlist.length} {t.tracks}</span>
                    </div>
                    ...
                    <div className="p-3 border-t border-slate-700 bg-slate-900">
                        <button onClick={generateNewTrack} className="w-full py-2 bg-pink-900/20 hover:bg-pink-900/40 text-pink-400 border border-pink-500/30 rounded-lg text-xs font-bold flex items-center justify-center gap-2">
                            <Activity className="w-3 h-3" /> {t.generate}
                        </button>
                    </div>
                </div>
            </div>

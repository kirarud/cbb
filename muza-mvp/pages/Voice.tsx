...
export const Voice: React.FC<VoiceProps> = ({ language, onVoiceInput }) => {
    ...
    const intonationLabel = isRu
        ? (intonation === 'QUESTION' ? 'Вопрос' : intonation === 'COMMAND' ? 'Команда' : 'Нейтрально')
        : intonation;
    ...
    const startMicrophone = async () => {
        try {
            ...
            let audioCtx = synthService.getContext();
            ...
            console.error(language === 'ru' ? "Ошибка микрофона" : "Mic Error", e);
        }
    };
        ...
        const animate = () => {
            ...
            if (analyserRef.current && audioContextRef.current) {
                ...
                if (pitch !== -1 && pitch > 50 && pitch < 2000) {
                    setDetectedHz(Math.round(pitch));
                    pitchHistory.push(pitch);
                    if (pitchHistory.length > 50) pitchHistory.shift();

                    // Detect Intonation Trend (Rise/Fall)
                    if (pitchHistory.length > 20) {
                        const start = pitchHistory[0];
                        const end = pitchHistory[pitchHistory.length - 1];
                        if (end > start * 1.2) setIntonation('QUESTION');
                        else if (end < start * 0.8) setIntonation('COMMAND');

...
export const CodeLab: React.FC<CodeLabProps> = ({ language, onLog, currentUser }) => {
    ...
    const DEFAULT_CODE_JS = isRu
        ? `// Песочница Muza JS
console.log(\"Ядро активно.\");
return \"Готово.\";`
        : `// Muza JS Sandbox
console.log(\"Kernel active.\");
return \"Ready.\";`;

    const DEFAULT_CODE_PY = isRu
        ? `# Локальный мост Muza
import pyautogui
import time
print(\"Двигаю мышь...\")
...
`;
    ...
    const DEFAULT_CODE_GENESIS = isRu
        ? `// СЕРВЕР ГЕНЕЗИС (Node.js)
const http = require('http');

const server = http.createServer((req, res) => {
  res.writeHead(200, { 'Content-Type': 'text/plain' });
  res.end('Привет из контейнера Генезиса Muza!');
});
...
`;
    ...
    const deployGenesisContainer = () => {
        setOutput([isRu ? "> Инициализация WebContainer..." : "> Initializing WebContainer environment...", isRu ? "> Запуск Node.js v18..." : "> Booting Node.js v18..."]);
        setContainerStatus('BOOTING');
        onLog(isRu ? "Запущена последовательность Генезиса." : "Genesis Sequence Initiated.", "INFO");

        setTimeout(() => {
            setOutput(prev => [...prev, isRu ? "> Установка зависимостей..." : "> Installing dependencies...", isRu ? "> Запуск серверного процесса..." : "> Starting server process..."]);
            setContainerStatus('RUNNING');
            setContainerUrl('http://localhost:3000');
            onLog(isRu ? "Контейнер материализован." : "Container Materialized.", "SUCCESS");
            
            // Simulate server logs
            setTimeout(() => setOutput(prev => [...prev, isRu ? "[СЕРВЕР] Запущен http://localhost:3000/" : "[SERVER] Server running at http://localhost:3000/"]), 1000);
            setTimeout(() => setOutput(prev => [...prev, isRu ? "[СЕРВЕР] Запрос: GET /" : "[SERVER] Request received: GET /"]), 3000);
        }, 2000);
    };
             ...
             <div className="flex items-center justify-between glass-panel p-4 rounded-xl shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-white flex items-center gap-2">
                            {mode === 'BRIDGE' ? t.localMode : mode === 'GENESIS' ? t.genesisTitle : t.browserMode}
                ...
                <div className="flex gap-2 items-center">
                    ...
                    >
                        ...
                        {mode === 'GENESIS' ? t.materialize : t.run}
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col md:flex-row gap-4 overflow-hidden min-h-0">
                ...
                <div className="flex-1 md:w-1/3 glass-panel rounded-xl overflow-hidden border border-slate-700 flex flex-col bg-black/50">
                    <div className="bg-slate-900 p-2 text-xs text-slate-500 font-mono border-b border-slate-800 flex justify-between items-center">
                        <span>{mode === 'GENESIS' ? t.containerLogs : t.output}</span>
                    ...
                    {mode === 'GENESIS' && containerStatus === 'RUNNING' ? (
                        <div className="flex-1 bg-white flex items-center justify-center flex-col">
                            <div className="text-black font-bold text-xl mb-2">{t.serverHello}</div>
                            <div className="text-slate-500 text-xs">{t.serverPort}</div>
                        </div>
                    ) : (
